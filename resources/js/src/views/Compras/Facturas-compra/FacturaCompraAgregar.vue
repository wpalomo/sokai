...
export default {
  ...
  methods: {
    ...
    guardarFact() {
      ...
      axios.post("/api/guardarfactura", {
        ...
        axios.post(urlxmlf,dataf).then(res => {
          ...
          var ruta_factura = "../server/" + this.usuario.id_empresa + "/comprobantes/retencion/" + this.nroAutorizacion + ".xml";
    ...
    obtenerComprobanteFirmado_sri(ruta_certificado, pwd_p12, ruta_respuesta, ruta_factura) {
      var response = [];
      axios.post("/api/leerFacturaphprc", {'ruta_factura': ruta_factura}).then(respuesta => {
          let me = this; 
          ...
          oReq.onload = function (oEvent) {
              ...
              axios.post("/api/firmaphprc", {'mensaje': comprobanteFirmado_xml,"id_empresa": me.usuario.id_empresa}).then(respuesta => {
                  var service = 'Validar Comprobante';
                  var xmlDoc = $.parseXML(me.contenido_comprobante),
                      $xml = $(xmlDoc),
                      $claveAcceso = $xml.find("claveAcceso"); 
                  axios.post("/api/validarComprobantephprc", {'service': service, 'claveAcceso': $claveAcceso.text(),"id_empresa": me.usuario.id_empresa}).then(respuestaValidarComprobante => {
                      ...
                      if (/RECIBIDA/i.test(respuesta) || /CLAVE ACCESO REGISTRADA/i.test(respuesta)) {
                        ...
                        xmlDoc = $.parseXML(me.contenido_comprobante),
                            $xml = $(xmlDoc),
                            $claveAcceso = $xml.find("claveAcceso");
                        axios.post("/api/autorizacionComprobantephprc", {'service': service, 'claveAcceso': $claveAcceso.text(),"id_empresa": me.usuario.id_empresa}).then(respuestaAutorizacionComprobante => {
                            var autorizacion_comprobante = respuestaAutorizacionComprobante.data;
                            var envioestado = "/api/respfacturarc";
                            ...
                            axios.post(envioestado, enviourl).then(res => {
                              ...
                              //me.$router.push("/compras/factura-compra");
                            }).catch(err => {
                              me.$vs.notify({
                                time:8000,
    ...
    firmarComprobante(mi_contenido_p12, mi_pwd_p12, comprobante) {
        ...
        var fechaInicio = cert.validity['notBefore'];
        var fechaFin = cert.validity['notAfter'];
        axios.post("/api/validarFechaCertificadophprc", {'fechaInicio': fechaInicio,'fechaFin': fechaFin}).then(respuesta => {
        });

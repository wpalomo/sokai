<template>
  <div id="invoice-page">
    <vx-card>
      ...
      <div class="vx-row leading-loose p-base">
        <div class="vx-col w-full mb-4" v-if="modofact==0">
          ...
          <div v-show="error" v-if="!id_cliente">
            <div v-for="err in errorcliente" :key="err" v-text="err" class="text-danger"></div>
          </div>
        </div>
      ...
      <div class="p-base">
          ...
          <template slot-scope="{data}">
            <vs-tr v-for="(tr, index) in data" :key="index">
                ...
                <vx-input-group>
                  ...
                  <template slot="append">
                    <div class="append-text btn-addon">
                      <button
                        class="botonstl"
                        :class="{'botonstl elejido':tr.p_descuento==1,'botonstl':tr.p_descuento!=1}"
                        @click="tr.p_descuento=1"
                      >$</button>
                      <button
                        class="botonstl"
                        :class="{'botonstl elejido':tr.p_descuento==0,'botonstl':tr.p_descuento!=0}"
              ...
              <vs-td
                ...
                v-model="tr.sub"
              >{{ (tr.cantidad*tr.precio-(((tr.cantidad*tr.precio)*tr.descuento)/100) )| currency }}</vs-td>
        ...
        <div class="vx-row mt-6">
          <div class="vx-col sm:w-1/2 w-full">
            <h6>Observaciones:</h6>
            <vs-textarea class="w-full" v-model="observacion" rows="2" :disabled="modofact!=0" />
          </div>
          <div class="vx-col sm:w-1/2 w-full">
                  ...
                  <vx-input-group>
                    ...
                    <template slot="append">
                      <div class="append-text btn-addon">
                        <button
                          class="botonstl"
                          :class="{'botonstl elejido':pp_descuento==1,'botonstl':pp_descuento!=1}"
                          @click="pp_descuento=1"
                        >$</button>
                        <button
                          class="botonstl"
                          :class="{'botonstl elejido':pp_descuento==0,'botonstl':pp_descuento!=0}"
      ...
      <transition name="slide-fade">
        <div class="vx-row leading-loose p-base" v-show="verretenciones">
              ...
              <div class="w-2/3 ml-auto mr-auto">
                <div class="vx-row">
                  <div class="vx-col md:w-3/5 w-full mb-2 ml-auto text-center">
                    ...
                    <vs-select
                      :disabled="totalpr<=0 || modofact !=0 || (totalef==100 && tr.iva==null)"
                  ...
                  <div class="flex-1 mb-2 mr-3 text-center">
                    <vs-input label="Valor" class="w-full" disabled v-model="ivapr12" />
                  </div>
                  <div class="flex-1 mb-2 mr-3 text-center">
                    <vs-input
                      label="Porcentaje"
                  ...
                  <div class="flex-1 mb-2 text-center">
                    <vs-input label="Cantidad" class="w-full" disabled v-model="tr.cantidadiva" />
                  </div>
                </div>
                <div class="vx-row">
          ...
          <div class="w-full" v-else>
            <div class="vx-row hovertrash" v-for="(tr,index) in valorretenciones" :key="index">
              <div class="w-2/3 ml-auto mr-auto">
                <div class="vx-row">
                  <div class="vx-col md:w-3/4 w-full mb-2 ml-auto text-center">
                    <label class="vs-input--label">Valores por IVA</label>
                    <vs-select
                      :disabled="totalpr<=0 || modofact !=0 || (totalef==100 && tr.iva==null)"
                      placeholder="Selecciona la retención"
        ...
        <div class="vx-row leading-loose p-base" v-show="verpagos">
          <div class="w-full" v-if="modofact==0">
            <div class="vx-row hovertrash" v-for="(tr,index) in valorpagos" :key="index">
              <div
                ...
                :class="{'sm:w-1/6': tr.metodo_pago!='' && tr.metodo_pago!='Efectivo' , 'sm:w-1/3': tr.metodo_pago=='' || tr.metodo_pago=='Efectivo'}"
              >
              ...
              <div
                ...
                v-if="tr.metodo_pago!='Tarjeta Credito' && tr.metodo_pago!='Otro' && tr.metodo_pago!='Efectivo' && tr.metodo_pago!=''"
              >
                <vs-input
                  :disabled="totalpr<=0 || modofact !=0"
                  class="w-full text-center"
              ...
              <div
                class="vx-col w-full mb-2 text-center"
                :class="{'sm:w-1/3': tr.metodo_pago!='' && tr.metodo_pago!='Efectivo'  , 'sm:w-1/2': tr.metodo_pago=='' || tr.metodo_pago=='Efectivo' }"
              >
          ...
          <div class="w-full" v-else>
            <div class="vx-row hovertrash" v-for="(tr,index) in valorpagos" :key="index">
              <div
                class="vx-col w-full mb-2 text-center"
                :class="{'sm:w-1/6': tr.forma_pago!='' , 'sm:w-1/3': tr.forma_pago==''}"
              >
                <label class="vs-input--label">Método de pago</label>
                <vs-select
                  :disabled="totalpr<=0 || modofact !=0"
              ...
              <div
                class="vx-col sm:w-1/3 w-full mb-2 text-center"
                v-if="tr.forma_pago!='Tarjeta Credito' && tr.forma_pago!='Otro' && tr.forma_pago!=''"
              >
                <vs-input
                  :disabled="totalpr<=0 || modofact !=0"
                  class="w-full text-center"
              ...
              <div
                class="vx-col w-full mb-2 text-center"
                :class="{'sm:w-1/3': tr.forma_pago!='' , 'sm:w-1/2': tr.forma_pago==''}"
              >
                <label class="vs-input--label">Comentario</label>
                <vs-textarea
                  :disabled="totalpr<=0 || modofact !=0"
      ...
      <div class="vx-col w-full" v-if="modofact==0">
        <vs-button
          ...
          @click="guardarp()"
        >Guardar</vs-button>
        <vs-button color="success" type="filled" v-else @click="guardar()">Guardar</vs-button>
        <!--<vs-button color="success" type="filled" @click="editar()">Guardar1</vs-button>-->
        <vs-button color="danger" type="filled" v-if="modofact==!$route.params.id" to="/facturacion/proforma">Cancelar</vs-button>
        <vs-button color="danger" type="filled" v-else to="/facturacion/factura-venta">Cancelar</vs-button>
      </div>
      <div class="vx-col w-full" v-else>
        <vs-button color="danger" type="filled" to="/facturacion/factura-venta">Regresar</vs-button>
      </div>
    </vx-card>
      ...
      for (var f = 0; f < this.valorretenciones.length; f++) {
        if (!this.$route.params.id) {
          ...
          if (
            ...
            valorr += parseFloat(this.valorretenciones[f].cantidadrenta);
          }
        }
      }
      ...
      if (this.$route.params.id && this.modofact != 0) {
        ...
        axios
          ...
          .then(res => {
            ...
            this.pago_credito = res.data.cuotas_totales / res.data.monto;
            if (res.data) {
                ...
                axios
                  ...
                  .then(respuestaValidarComprobante => {
                    ...
                    if (
                      ...
                      /CLAVE ACCESO REGISTRADA/i.test(respuesta)
                    ) {
                      service = "Autorizacion Comprobante";
                      (xmlDoc = $.parseXML(me.contenido_comprobante)),
                        ($xml = $(xmlDoc)),
                        ($claveAcceso = $xml.find("claveAcceso"));
                      axios
                        .post("/api/autorizacionComprobantephp", {
                          service: service,
                          claveAcceso: $claveAcceso.text(),
                          id_empresa: me.usuario.id_empresa
                        })
                        .then(respuestaAutorizacionComprobante => {

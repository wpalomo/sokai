<template>
  <div id="simple-calendar-app">
    <div class="vx-card app-fixed-height" style="min-height: 500px;">
      <calendar-view
        ref="calendar"
        ...
        <div slot="header" class="mb-4">
          ...
          <div class="vx-row no-gutter">
            ...
            <div class="vx-col w-1/3 items-center sm:flex hidden">
              ...
              <vs-button icon-pack="feather" icon="icon-plus" @click="promptAddNewEvent(new Date())"> {{ $t('anadir') }} </vs-button>
            </div>
    ...
    <vs-prompt
        ...
        v-bind:title= anadirevento
        v-bind:accept-text= anadirevento
        @accept="addEvent"
        :is-valid="validForm"
        ...
        <vs-input name="event-name" v-validate="'required'" class="w-full" v-bind:label-placeholder="tituloevento" v-model="title"></vs-input>
        <div class="my-4">
            <small class="date-label">{{ $t("iniciodata") }}</small>
            <datepicker name="start-date" v-model="startDate" :disabled="disabledFrom"></datepicker>
        </div>
        <div class="my-4">
            <small class="date-label">{{ $t("findata") }}</small>
            <datepicker :disabledDates="disabledDatesTo" name="end-date" v-model="endDate"></datepicker>
        </div>
        <vs-input name="event-url" v-validate="'url'" class="w-full mt-6" v-bind:label-placeholder="urlevento" v-model="url" :color="!errors.has('event-url') ? 'success' : 'danger'"></vs-input>
    ...
    <vs-prompt
        class="calendar-event-dialog"
        v-bind:title = editarevento
        v-bind:accept-text= editarboton
        v-bind:cancel-text = removerboton
...
export default {
  ...
  data() {
    return {
      ...
      endDate: '',
      labelLocal: 'none',
      tituloevento: this.$t("tituloevento"),
      urlevento: this.$t("urlevento"),
      anadirevento: this.$t("anadirevento"),
      editarevento: this.$t("editarevento"),
      editarboton: this.$t("editarboton"),
      removerboton: this.$t("removerboton"),
      url: '',
      calendarView: 'month',
  ...
  computed: {
    ...
    labelColor() {
        return (label) => {
            if (label == "Negocios") return "success"
            else if (label == "Trabajo") return "warning"
